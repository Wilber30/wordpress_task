...
class Home {
    ...
    public function created_shortcode() {
        ...
        foreach ($this->database_data() as $value) {
            ...
            $return .= '<tr>';
            $return .= '<td>' . $id . '</td>';
            $return .= '<td>' . $this->name_converter($value['name']) . '</td>';
            $return .= '<td>' . $this->name_converter($value['style_name']) . '</td>';
            $return .= '<td><span>Shortcode &nbsp;&nbsp;<input type="text" onclick="this.setSelectionRange(0, this.value.length)" value="[oxilab_flip_box id=&quot;' . $id . '&quot;]"></span> <br>'
